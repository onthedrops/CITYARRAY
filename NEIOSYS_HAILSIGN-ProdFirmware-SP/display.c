...
void DISPLAY_Bitmap_8Bit(DISPLAY_BITMAP_8BIT bitmap)
{
  ...
  bitmap.nColumns = MIN(bitmap.nColumns, signConfig.signWidth);
...
DISPLAY_BITMAP_1BIT *Convert_SBitmap(DISPLAY_SBITMAP_1BIT *bitmap)
{
  ...
  if(cols < signConfig.signWidth) {
     cols = signConfig.signWidth;
  }
...
{
    int cols = signConfig.signWidth;
    int local_end = 0;
    
     if(offset > (signConfig.signWidth - 1)) {
        local_end = offset - (signConfig.signWidth - 1);
     } else {
        local_end = 0;
     }
...
#ifdef SCROLLOG
    char s[64];
    sprintf(s,"offset %d le: %d cols: %d srcs: %d",offset, local_end, cols, screenRed.nColumns);
    slog(s);
#endif
